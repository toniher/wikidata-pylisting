...
parser = argparse.ArgumentParser(
    description="""Script for generating evolution stats"""
)
# TODO: Need to add wikilang stuff here
parser.add_argument(
...
def printDfoWiki(df, site, summary, targetpage):
    ...
    for id, article in df.itertuples(index=False):
        text = (
            text
            + "|-\n| "
            + formatWikidata(str(id))
            + " || "
            + formatCawiki(str(article))
            + "\n"
        )
...
# Get all bios
bios = pd.read_sql_query(
    "SELECT w.id as id, w.article as article, g.gender as gender, b.cdate as cdate, b.cuser as cuser from `bios` b, `wikidata` w "
    + extratable
    + " where b.article=w.article "
    + extrawhere
    + extrasort,
    conn,
)
...
merged = pd.merge(
    numarticles, numarticlesw, how="left", on=["year", "month"], suffixes=("", "w")
)
...
wnumarticlesw = (
    women.groupby(["year", "month", "week"])["article"].count().reset_index()
)
...
text = (
    text
    + "{{Graph:Chart|width=600|height=200|type=line|colors=purple|xAxisAngle = -40|x="
    + ",".join(timeperiods)
    + "|y="
    + ",".join(accwa)
    + "}}\n"
