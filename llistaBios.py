...
if "mysql" in data:
	conn = MySQLdb.connect(host=data["mysql"]["host"], user=data["mysql"]["user"], passwd=data["mysql"]["password"],
	                       db=data["mysql"]["database"], use_unicode=True, charset='utf8mb4', init_command='SET NAMES utf8mb4')
...
query = """SELECT ?item ?genere ?article WHERE {
?item wdt:P31 wd:Q5.
?article schema:about ?item .
?article schema:isPartOf <https://{1}/> .
#SERVICE wikibase:label { bd:serviceParam wikibase:language "[AUTO_LANGUAGE],{2}" } .
OPTIONAL {
?item wdt:P21 ?genere .
}
} ORDER BY ?article
""".format(host, wikilang)
