...
parser = argparse.ArgumentParser(
    description="""Script for parsing authorities from dumps and DB"""
)
...
def addToDb(id, props, conn, iter):
    ...
    c.executemany(
        "INSERT INTO `authorities` (`id`, `authority`) VALUES ( %s, %s )", records
    )
...
if "dump" in args:
    if args.dump is not None:
        ...
        cur.execute(
            "CREATE TABLE IF NOT EXISTS `authorities` (  `id` VARCHAR(25), `authority` VARCHAR(25), PRIMARY KEY (`id`, `authority`) ) ;"
        )
        ...
        cur.execute(
            "CREATE TABLE IF NOT EXISTS `authtypes` (  `name` VARCHAR(25), `desc` VARCHAR(125), `prop` VARCHAR(25), `authtype` int(2), PRIMARY KEY (`prop`) ) ;"
        )
        ...
        for prop in authtypes:
            cur.execute(
                "INSERT INTO `authtypes` (`name`, `desc`, `prop`, `authtype`) VALUES ( %s, %s, %s, %s )",
                [
                    authtypes[prop][0],
                    authtypes[prop][1],
                    authtypes[prop][2],
